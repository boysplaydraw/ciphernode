...
Docker kurulumu varsayılan olarak Tor süreci çalıştırmaz. Daha stabil model, Tor hidden service'i host işletim sisteminde veya CasaOS üzerinde ayrı servis olarak çalıştırıp CipherNode web/API portlarına yönlendirmektir.

Örnek Tor hidden service yönlendirmesi:

```text
HiddenServiceDir /var/lib/tor/ciphernode/
HiddenServicePort 80 127.0.0.1:8080
HiddenServicePort 5000 127.0.0.1:5000
```

Tor'un ürettiği hostname'i compose ortamına ekleyin:

```env
ONION_ADDRESS=abc123...xyz.onion
```

Bu adres `GET /api/onion-address` endpoint'inden döner ve uygulamanın Ağ Ayarları ekranında görünür.
...
| `TOR_ENABLED` | `false` | Eski ayar; Docker image içinde Tor daemon başlatmaz |
| `ONION_ADDRESS` | boş | Host üzerinde kurulan harici Tor hidden service adresi; `/api/onion-address` ile istemcilere ilan edilir |
