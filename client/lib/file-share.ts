...
export async function shareFile(params: {
  ...
  if (checkSize > RELAY_MAX) {
    throw new Error(
      `Dosya relay için çok büyük. Maksimum: 100 MB (mevcut: ${(checkSize / (1024 * 1024)).toFixed(1)} MB). Büyük dosyalar P2P ile gönderilir.`,
    );
  }

  const actualFileName = rawFile?.name || fileNameParam || "file";
  const actualMimeType =
    rawFile?.type || fileMimeTypeParam || "application/octet-stream";
  ...
  const file =
    rawFile && scrubMetadata ? await scrubFileMetadata(rawFile) : rawFile;
...
export async function downloadAndDecryptFile(params: {
  ...
  const response = await fetch(
    new URL(`api/files/${fileId}`, apiUrl).toString(),
  );
