...
import { parseContactId, parsePublicKeyFingerprint } from "@/lib/crypto";
import {
  addContact,
  getContacts,
  updateContact,
...
export default function AddContactScreen() {
  ...
  const parseContactInput = useCallback(() => {
    const raw = contactIdInput.trim();
    if (raw.startsWith("{")) {
      const parsed = JSON.parse(raw);
      const id = parseContactId(String(parsed.id || ""));
      if (!id) return null;
      return {
        id,
        publicKey: String(parsed.pk || parsed.publicKey || ""),
        nostrPubkey: String(parsed.npk || parsed.nostrPubkey || ""),
      };
    }
    const id = parseContactId(raw);
    return id ? { id, publicKey: "", nostrPubkey: "" } : null;
  }, [contactIdInput]);
  ...
  const handleAddContact = useCallback(async () => {
    ...
    try {
      parsedContact = parseContactInput();
    } catch {
      parsedContact = null;
    }
    const parsedId = parsedContact?.id ?? null;
    if (!parsedId) {
    ...
    try {
      ...
      let publicKey = parsedContact?.publicKey ?? "";
      let nostrPubkey = parsedContact?.nostrPubkey ?? "";
      let keySource = "none";
  ...
  }, [parseContactInput, identity, t, navigation, language]);
