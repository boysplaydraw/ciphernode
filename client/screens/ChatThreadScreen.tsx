  ...
  useEffect(() => {
    const decrypt = async () => {
      ...
      if (isEncrypted) {
        ...
        setVerified(senderPublicKey ? result.verified : null);
      } else {
        setDisplayContent(message.content);
        setVerified(null);
      }
        ...
        {message.expiresAt ? (
          ...
          <Feather
            name={
              verified === false
                ? "alert-triangle"
                : !isMine && !contact?.publicKey
                  ? "help-circle"
                  : verified === true && !isMine
                    ? "shield"
                    : "lock"
            }
            size={10}
...
export default function ChatThreadScreen() {
  ...
  const p2pPendingOffer = useRef<Map<string, RTCSessionDescriptionInit>>(
    new Map(),
  );
  ...
  const canEncryptToContact = !!contact?.publicKey;
  ...
  React.useLayoutEffect(() => {
    const displayName = contact?.displayName || contact?.id || contactId;
  ...
  const startP2PReceive = useCallback(
    async (meta: P2PFileOffer, offerSdp: RTCSessionDescriptionInit) => {
      ...
      try {
            ...
            setIncomingFiles((prev) =>
              prev.map((f) =>
                f.fileId === fakeId
                  ? {
                      ...f,
                      p2pBuffer: buffer ?? undefined,
                      p2pUri: fileUri ?? undefined,
                    }
                  : f,
              ),
            );
        ...
        Alert.alert(
          "P2P Hata",
          err instanceof Error ? err.message : "Dosya alınamadı",
        );
      }
    },
    [],
  ...
  const handleSendTorrent = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(
        "Torrent Hata",
        err instanceof Error ? err.message : "Torrent oluşturulamadı",
      );
    } finally {
      setSendingFile(false);
    }
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    if (!contact.publicKey) {
      Alert.alert(
        "Anahtar Bekleniyor",
        "Bu kişi için açık anahtar yok. Mesajı güvenli göndermek için kişiyi QR koduyla ekleyin veya karşı taraf uygulamaya bağlıyken tekrar deneyin.",
        [{ text: "Tamam" }],
      );
      return;
    }
      ...
      >
        ...
        {(sentFiles.length > 0 || incomingFiles.length > 0) && (
          <View style={styles.incomingFilesContainer}>
            {sentFiles.map((file) => (
              <View
                key={`sent-${file.fileId}`}
                style={[styles.incomingFileRow, styles.sentFileRow]}
              >
                <Feather
                  name="upload"
                  size={14}
                ...
                <View style={styles.sentBadge}>
                  <ThemedText style={styles.sentBadgeText}>
                    ✓ Gönderildi
                  </ThemedText>
                </View>
              </View>
        ...
        {contact && !canEncryptToContact ? (
          <View style={styles.keyWarning}>
            <Feather name="shield-off" size={14} color={Colors.dark.warning} />
            <ThemedText style={styles.keyWarningText}>
              Bu kişi için açık anahtar yok. Mesaj ve dosya göndermek için QR
              koduyla ekleyin veya kişi çevrimiçiyken anahtarın senkronize
              olmasını bekleyin.
            </ThemedText>
          </View>
        ) : null}
        ...
        <View style={[styles.inputContainer, { paddingBottom: bottomPadding }]}>
          <Pressable
            ...
            style={({ pressed }) => [
              styles.attachButton,
              pressed && { opacity: 0.7 },
              !canEncryptToContact && { opacity: 0.4 },
            ]}
          >
            <Feather
              ...
              size={20}
              color={
                sendingFile || !canEncryptToContact
          ...
          {isElectron() && (
            ...
            >
              <Feather
                ...
                size={18}
                color={
                  sendingFile ? Colors.dark.textDisabled : Colors.dark.primary
                }
              />
            </Pressable>
          )}
          ...
          <Pressable
            ...
            disabled={!inputText.trim() || !canEncryptToContact}
            style={({ pressed }) => [
              styles.sendButton,
              (!inputText.trim() || !canEncryptToContact) &&
...
const styles = StyleSheet.create({
  ...
  keyWarning: {
    ...
    borderTopWidth: 1,
    borderTopColor: Colors.dark.warning + "44",
    paddingHorizontal: Spacing.lg,
    paddingVertical: Spacing.sm,
  },
  keyWarningText: {
