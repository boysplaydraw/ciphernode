        ...
        <View style={styles.messageFooter}>
          {isP2P && (
            <Feather
              ...
              size={10}
              color={
                isOwn ? Colors.dark.buttonText + "99" : Colors.dark.success
              }
              style={{ marginRight: 4 }}
            />
          )}
...
export default function GroupThreadScreen() {
  ...
  useEffect(() => {
    const unsub = onGroupP2PMessage(
      async (from, gId, content, encrypted, timestamp) => {
  ...
  const handleSend = async () => {
    ...
    if (relayOnline) {
      ...
      const others = group.members.filter((m) => m.id !== identity.id);
