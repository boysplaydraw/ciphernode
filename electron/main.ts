...
function getWebTorrentClient() {
  if (!_webtorrentClient) {
    try {
      ...
      throw new Error(
        "WebTorrent kurulu değil. Electron için: npm install webtorrent",
      );
    }
  }
...
function setupWebTorrentIPC() {
  ...
  ipcMain.handle(
    "webtorrent:download",
    async (_event, params: { magnetURI: string }, ...args: any[]) => {
  ...
  autoUpdater.on("update-downloaded", (info) => {
    ...
    dialog
      .showMessageBox({
        ...
        detail:
          "Uygulamayı yeniden başlatarak güncellemeyi uygulayabilirsiniz.",
