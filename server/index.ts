...
function setupBodyParsing(app: express.Application) {
  ...
  app.use(
    express.urlencoded({ extended: false, limit: `${maxFileSizeMb + 50}mb` }),
  );
}
...
function resolveSSLPaths(): { cert: string; key: string } | null {
  ...
  if (fs.existsSync(path.resolve(process.cwd(), ".env"))) {
    try {
      ...
      for (const line of envContent.split("\n")) {
        ...
        if (k === "SSL_DOMAIN" && !process.env.SSL_DOMAIN)
          process.env.SSL_DOMAIN = v;
      }
    } catch {
      /* .env okuma hatası — sessizce geç */
    }
  }
  ...
  if (dom) {
    ...
    if (fs.existsSync(winSslDir)) {
      ...
      const certFile =
        files.find(
          (f) =>
            f.includes(dom) &&
            (f.includes("chain") || f.includes("cert")) &&
            f.endsWith(".pem"),
    ...
    log(
      `[SSL]   Windows: powershell -File scripts\\setup-ssl-windows.ps1 -Domain ${dom}`,
    );
...
(async () => {
  ...
  if (sslPaths) {
    ...
    const httpRedirectPort = parseInt(
      process.env.HTTP_REDIRECT_PORT || "80",
      10,
    );
    http.createServer(redirectApp).listen(httpRedirectPort, host, () => {
      log(
        `[SSL] HTTP→HTTPS yönlendirme sunucusu port ${httpRedirectPort} üzerinde çalışıyor`,
      );
    });

    log(`[SSL] Sertifika: ${sslPaths.cert}`);
    log(
      `[SSL] İpucu: Certbot otomatik yenileme için 'sudo certbot renew --quiet' cron'a ekleyin.`,
    );
  } else {
    ...
    server.listen(port, host, async () => {
      ...
      if (process.env.SSL_DOMAIN) {
        log(
          `[SSL] SSL etkinleştirmek için: sudo bash scripts/setup-ssl.sh ${process.env.SSL_DOMAIN}`,
        );
      }
      ...
      if (process.env.TOR_ENABLED === "true" || process.env.ONION_ADDRESS) {
        ...
        if (onion) {
          ...
          log(
            "[Tor] Hidden service başlatılamadı — normal HTTP modunda devam ediliyor.",
          );
        }
      }
    });
