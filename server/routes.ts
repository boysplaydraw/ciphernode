...
const FILE_TTL = parseInt(
  process.env.FILE_TTL_MS || String(24 * 60 * 60 * 1000),
  10,
);
const MAX_FILE_SIZE =
...
const MESSAGE_TTL = parseInt(
  process.env.MESSAGE_TTL_MS || String(24 * 60 * 60 * 1000),
  10,
);
...
export async function registerRoutes(
  app: Express,
  existingServer?: Server | HttpsServer,
  ...
  io.on("connection", (socket) => {
    ...
    socket.on(
      "webrtc:offer",
      (data: { peerId: string; sdp: unknown; from: string }) => {
    ...
    socket.on(
      "webrtc:ice",
      (data: { peerId: string; candidate: unknown; from: string }) => {
        const targetSocket = connectedUsers.get(data.peerId);
        if (targetSocket) {
