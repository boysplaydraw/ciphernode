...
function generateAlias(): string {
  const adj = ["ghost", "silent", "neon", "void", "steel", "prism", "cipher", "lunar", "dark", "binary"];
  const noun = ["sparrow", "fox", "wolf", "raven", "echo", "hawk", "storm", "drift", "tide", "shift"];
...
const FILE_TTL = 24 * 60 * 60 * 1000;  // 24 saat
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/files/upload", (req, res) => {
    ...
    relayLog(`File uploaded: ${name} (${file.size} bytes)`, "Event: file_uploaded");
  ...
  app.get("/api/files/:fileId", (req, res) => {
    ...
    file.downloadCount++;
    relayLog(`File downloaded: ${file.name} (${file.downloadCount}/${file.maxDownloads})`, "Event: file_downloaded");
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("register", (data: { userId: string; publicKey?: string; torEnabled?: boolean; groups?: string[] } | string) => {
      ...
      socket.broadcast.emit("user:online", { userId, publicKey: publicKey || "" });
    ...
    socket.on("message", (data: { to: string; from: string; encrypted: string; id?: string }) => {
      ...
      if (deliveredMessageIds.has(messageId)) {
        relayLog(`Duplicate message ignored: ${messageId}`, "Duplicate message ignored");
        return;
      }
      ...
      if (targetSocketId) {
        ...
        // Production'da kimlik bilgisi loglanmaz
        relayLog(`Message delivered: ${data.from} -> ${data.to}`, "Event: message_delivered");
      } else {
        ...
        relayLog(`Message queued for offline user: ${data.to}`, "Event: message_queued");
      }
    });
    ...
    socket.on("group:join", (data: { groupId: string; userId: string }) => {
      ...
      socket.join(data.groupId);
      relayLog(`User ${data.userId} joined group: ${data.groupId}`, "Event: group_join");
    });
    ...
    socket.on("group:leave", (data: { groupId: string; userId: string }) => {
      ...
      relayLog(`User ${data.userId} left group: ${data.groupId}`, "Event: group_leave");
    });
    ...
    socket.on("group:message", (data: { groupId: string; from: string; encrypted: string; content: string; id?: string }) => {
      ...
      if (deliveredMessageIds.has(messageId)) {
        relayLog(`Duplicate group message ignored: ${messageId}`, "Duplicate group message ignored");
        return;
      }
      deliveredMessageIds.set(messageId, Date.now());
    ...
    socket.on("matching:start", (data: { userId: string }) => {
      ...
      const me: MatchingUser = { userId, socketId: socket.id, alias, joinedAt: Date.now() };
    ...
    socket.on("matching:accept", (data: { userId: string; sessionId: string }) => {
      ...
      if (session.acceptedBy.size >= 2) {
        ...
        socket.join(roomId);
        io.to(session.userA.socketId).emit("matching:connected", { sessionId: data.sessionId, roomId });
        io.to(session.userB.socketId).emit("matching:connected", { sessionId: data.sessionId, roomId });
        relayLog("Matching: both accepted, connected", "Event: matching_connected");
      } else {
        ...
        relayLog("Matching: one side accepted", "Event: matching_one_accepted");
      }
    });

    /** Kullanıcı eşleşmeyi reddediyor */
    socket.on("matching:decline", (data: { userId: string; sessionId: string }) => {
      const session = matchSessions.get(data.sessionId);
      if (!session || session.state === "ended") return;
    ...
    socket.on("matching:message", (data: { sessionId: string; userId: string; encrypted: string }) => {
      const session = matchSessions.get(data.sessionId);
    ...
    socket.on("file:share", (data: {
      ...
      if (targetSocketId) {
        ...
        relayLog(`File link relayed: ${data.fileName}`, "Event: file_link_relayed");
      } else {
    ...
    socket.on("matching:file_share", (data: {
      sessionId: string;
      userId: string;
      fileId: string;
      fileName: string;
      ...
      relayLog(`Matching file share: ${data.fileName}`, "Event: matching_file_share");
    });

    /** Kullanıcının public key'ini gerçek zamanlı sorgula */
    socket.on("user:lookup", (data: { userId: string }, callback: (result: { publicKey: string | null }) => void) => {
    ...
    socket.on("matching:end_session", (data: { userId: string; sessionId: string }) => {
      const session = matchSessions.get(data.sessionId);
      if (!session) return;
