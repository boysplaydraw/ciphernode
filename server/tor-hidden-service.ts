...
function prepareTorConfig(hiddenServicePort: number): {
  torrcPath: string;
  dataDir: string;
} {
...
export async function startTorHiddenService(
  serverPort: number,
): Promise<string | null> {
  ...
  try {
    const child = spawn(torBin, ["-f", torrcPath], {
    ...
    if (onionAddress) {
      ...
      log(
        "UYARI: .onion adresi okunamadı, birkaç saniye sonra tekrar denenecek.",
      );
